...
def dynamic_bwrand(mix_pool, config):
    ...
    for e in range(config["epoch_num"]):   
        ...
        mng.mix_select('bw', mixnet, config["construction"]["sample_fraction"], config["network"]["setting"])
...
def static_bwrand(mix_pool, config):
